...
export default function Registration({}: Props) {
  ...
  const handleOutsideClick = (e: any) => {
    if (
      ...
      !registrationFormRef.current.contains(e.target)
    ) {
      router.push("/lobby");
    }
  };
  ...
  return (
    <>
      <div
        className="fixed top-0 left-0 w-screen h-screen flex justify-center items-center bg-black bg-opacity-60 z-50"
        onClick={handleOutsideClick}
      >
        <div ref={registrationFormRef}>
          <RegistrationForm
            popup={true}
            closePopup={() => router.push("/lobby")}
            router={router}
